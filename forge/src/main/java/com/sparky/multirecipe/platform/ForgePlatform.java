...
import com.sparky.multirecipe.common.PolymorphForgePacketDistributor;
...
public class ForgePlatform implements IPlatform {
  ...
  public boolean isShaped(RecipeHolder<?> recipe) {
    return recipe.value() instanceof IShapedRecipe;
  }
  ...
  public boolean isSameShape(RecipeHolder<?> recipe1, RecipeHolder<?> recipe2) {
    ...
    if (isShaped(recipe1) && isShaped(recipe2)) {
      IShapedRecipe<?> shaped = (IShapedRecipe<?>) recipe1.value();
      IShapedRecipe<?> otherShaped = (IShapedRecipe<?>) recipe2.value();
